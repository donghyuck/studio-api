...
| `keywords` | 문서 단위 keyword 목록. `studio.ai.rag.keywords.scope=document|both`일 때 기록 |
...
기본 `PcaVectorProjectionGenerator`는 추가 의존성 없이 PCA 좌표를 계산한다. `UmapVectorProjectionGenerator`와
`TsneVectorProjectionGenerator`는 PCA 초기 좌표를 embedding cosine distance 기반 이웃 보존 방식으로 보정하는 서버 측 deterministic 구현이다.
운영 화면은 `ProjectionAlgorithm` 값과 무관하게 저장된 `x`, `y` 좌표만 사용한다. 더 높은 품질의 알고리즘이
필요하면 동일한 `VectorProjectionGenerator` SPI로 교체한다. 화면 API는 원본 embedding 값을 반환하지 않으며,
metadata는 표시용 allowlist로 제한한다. 원문 text는 벡터 항목 상세 조회에서만 제공한다.
