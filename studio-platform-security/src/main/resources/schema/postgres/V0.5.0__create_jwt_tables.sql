...
CREATE TABLE IF NOT EXISTS TB_APPLICATIONI_REFRESH_TOKEN (
...
ALTER TABLE TB_APPLICATIONI_REFRESH_TOKEN
...
ALTER TABLE TB_APPLICATIONI_REFRESH_TOKEN
  ...
  REFERENCES TB_APPLICATIONI_REFRESH_TOKEN (ID)
  ON DELETE SET NULL;
...
-- 조회/정리 성능 인덱스
CREATE INDEX IF NOT EXISTS IX_REFRESH_USER        ON TB_APPLICATIONI_REFRESH_TOKEN (USER_ID);
CREATE INDEX IF NOT EXISTS IX_REFRESH_EXPIRES_AT  ON TB_APPLICATIONI_REFRESH_TOKEN (EXPIRES_AT);

-- 활성 토큰 조회 최적화(부분 인덱스)
CREATE INDEX IF NOT EXISTS IX_REFRESH_ACTIVE
  ON TB_APPLICATIONI_REFRESH_TOKEN (USER_ID, EXPIRES_AT)
  WHERE REVOKED = FALSE;