...
ALTER TABLE tb_skill_cluster
    ...
    ADD COLUMN IF NOT EXISTS representative_skill_ids JSONB,
...
CREATE TABLE IF NOT EXISTS tb_skill_cluster_member (
    ...
    distance_to_centroid DOUBLE PRECISION NOT NULL DEFAULT 0,
    is_representative BOOLEAN NOT NULL DEFAULT FALSE,
    PRIMARY KEY (projection_id, cluster_id, skill_id)
);

CREATE INDEX IF NOT EXISTS idx_skill_dictionary_skill_type
    ON tb_skill_dictionary(skill_type, status, name);

CREATE INDEX IF NOT EXISTS idx_skill_projection_run_lookup
    ON tb_skill_projection(skill_type, embedding_provider, embedding_model, embedding_dimension);

CREATE INDEX IF NOT EXISTS idx_skill_cluster_job
    ON tb_skill_cluster(job_id, skill_type);

CREATE INDEX IF NOT EXISTS idx_skill_cluster_member_cluster
