...
class OnTemplatePersistenceCondition extends SpringBootCondition {

    @Override
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        Map<String, Object> attributes =
                metadata.getAnnotationAttributes(ConditionalOnTemplatePersistence.class.getName());
        Type expected = (Type) attributes.get("value");
        Type actual = resolve(context.getEnvironment());
        if (actual == expected) {
            return ConditionOutcome.match("Template persistence matched " + expected);
        }
        return ConditionOutcome.noMatch("Template persistence was " + actual + ", expected " + expected);
    }

    private Type resolve(Environment env) {
        Type configured = parse(env.getProperty(PropertyKeys.Features.PREFIX + ".template.persistence"));
        if (configured != null) {
            return configured;
        }
