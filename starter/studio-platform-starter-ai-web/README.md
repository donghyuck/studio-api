...
객체 단위 색인을 제거하려면 `DELETE /api/mgmt/ai/rag/objects/{objectType}/{objectId}`를 호출한다.
이 API는 해당 object scope의 chunk/vector/metadata를 삭제하며, `services:ai_rag write` 권한이 필요하다.
...
`data.indexed=false`를 반환하고 chunk page API는 빈 `items`를 반환한다.
