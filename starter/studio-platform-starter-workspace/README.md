...
company scope를 DB 제약까지 강제하는 운영 환경은 `COMPANY_ID NOT NULL`, company-scoped unique 제약, company foreign key를 전제로 합니다. 이 구성에서는 `studio.features.workspace.company-required=true`, `studio.features.workspace.company-scope-enforced=true`를 함께 설정합니다. `company-scope-enforced=true`는 `company-required=true`와 함께만 사용할 수 있고, runtime root slug 중복 체크를 company scope로 전환합니다. 필요한 DB 제약이 없거나 강제 DB 제약이 있는 상태에서 `company-scope-enforced=false`로 기동하면 애플리케이션은 tenant-ambiguous `path` 조회를 막기 위해 실패합니다.
...
자동구성 import는 workspace 모듈의 새 패키지 구조를 기준으로 한다.

- contract: `studio.one.platform.workspace.application.*`, `studio.one.platform.workspace.domain.*`
- default implementation: `studio.one.platform.workspace.application.service`, `studio.one.platform.workspace.infrastructure.persistence.jpa`
- web DTO: `studio.one.platform.workspace.web.dto.request`, `studio.one.platform.workspace.web.dto.response`
