...
첨부 업로드 검증은 `ObjectTypeRuntimeService`를 사용한다. 따라서 attachment starter만 단독으로 추가하지 말고,
기반 starter와 objectType starter를 함께 추가해야 한다.
...
| `AttachmentController`<br>`AttachmentMgmtController`<br>`AttachmentAuditMgmtController`<br>`MeAttachmentController` | 각 컨트롤러 | `web.enabled=true` |
...
- `AttachmentDownloadUrlService`는 `studio.attachment.download-url.public-base-url`과 32 bytes 이상 `signing-secret`으로 application-level signed link를 발급한다.
- `GET /api/attachments/signed-download`는 token-only 공개 링크이므로 전역 `/api/**` 인증 필터를 사용하는 애플리케이션은 해당 경로를 `permitAll`에 추가한다.
...
`POST /{attachmentId}/download-url`은 `{publicBaseUrl}/api/attachments/signed-download?token=...` 형식의 application signed link를 반환한다.
`GET /signed-download?token=...`은 token-only로 검증 후 `AttachmentService.getInputStream()` 경로로 스트리밍한다.
...
- signed download URL 발급/접근 감사 로그 조회를 쓰려면 `TB_APPLICATION_ATTACHMENT_URL_ISSUE_LOG`, `TB_APPLICATION_ATTACHMENT_DOWNLOAD_LOG`, `features:attachment_download_url_issue_audit/read`, `features:attachment_download_audit/read` 권한 정책을 준비한다.
