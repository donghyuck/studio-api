...
`studio-platform-ai`는 공통 계약만 제공하고, LangChain4j adapter와 기본 RAG 구현은 이 스타터가 제공한다.
...
스타터는 OpenAI, Google AI Gemini, Ollama용 LangChain4j provider artifact를 직접 포함한다.
소비 앱은 별도 Spring AI BOM이나 Spring AI provider starter를 선언하지 않는다.
...
provider SDK의 실제 API key, model, dimension, base-url 같은 실행 옵션은
`studio.ai.providers.<id>.*`를 canonical source로 둔다. `spring.ai.*`는 1.x 호환을 위한
migration fallback으로만 지원한다. `langchain4j.*` 설정 namespace는 운영 표준으로 노출하지 않는다.

RAG 색인/검색에서 embedding provider/model을 고정해 운영해야 하면 `studio.ai.rag.embedding-profiles`
를 사용한다. profile은 Studio orchestration 설정이며 provider SDK의 API key/model/dimension은
`studio.ai.providers.<id>.*`가 소유하고 `spring.ai.*`는 fallback으로만 사용한다.
...
LangChain4j adapter는 provider 등록 시점의 embedding model을 사용하므로, profile/request의 `model`은 실제
provider 설정과 일치해야 한다. 다른 model이 들어오면 metadata만 다른 vector space로 기록되는 것을 막기 위해
...
| `modelName` | provider metadata model name |
...
`ChatPort.stream(ChatRequest)`는 provider streaming 지원 여부와 관계없이 기존 `chat()` 호출 결과를 `delta`, `usage`, `complete` event sequence로 변환하는 fallback을 사용한다.
...
이 스타터는 현재 LangChain4j provider adapter와 pgvector `VectorStorePort` adapter를 함께 자동 구성한다.
...
- `studio.ai.providers.<id>.*`가 provider SDK API key, base-url, model, embedding dimension의 canonical source다.
  `spring.ai.*`는 1.x migration fallback으로만 읽는다. `langchain4j.*` namespace는 운영 설정으로 노출하지 않는다.
