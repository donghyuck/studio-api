...
class OnLoginFailurePersistenceCondition extends SpringBootCondition {

    @Override
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        Map<String, Object> attributes =
                metadata.getAnnotationAttributes(ConditionalOnLoginFailurePersistence.class.getName());
        Type expected = (Type) attributes.get("value");
        Type actual = resolve(context.getEnvironment());
        if (actual == expected) {
            return ConditionOutcome.match("LoginFailure persistence matched " + expected);
        }
        return ConditionOutcome.noMatch("LoginFailure persistence was " + actual + ", expected " + expected);
    }
