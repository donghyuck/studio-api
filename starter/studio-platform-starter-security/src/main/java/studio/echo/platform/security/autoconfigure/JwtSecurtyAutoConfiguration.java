...
import studio.echo.base.security.jwt.refresh.*;
import studio.echo.base.security.web.controller.JwtAuthController;
import studio.echo.base.security.web.controller.JwtRefreshController;
...
public class JwtSecurtyAutoConfiguration {
    ...
    private static final String DEFAULT_ENTITY_PACKAGE = RefreshTokenStore.class.getPackageName();
    ...
    public Clock jwtClock(ObjectProvider<I18n> i18nProvider) {
        ...
        log.info(LogUtils.format(i18n, I18nKeys.AutoConfig.Feature.Service.DETAILS, FEATURE_NAME, LogUtils.blue(Clock.class, true), LogUtils.red(State.CREATED.toString())));
    ...
    public JwtRefreshController jwtRefreshEndpoint(
        ...
        log.info(LogUtils.format(i18n, I18nKeys.AutoConfig.Feature.EndPoint.REGISTERED, FEATURE_NAME,
                ...
                props.getEndpoints().getBasePath(), getModeString(props)));
        return new JwtRefreshController(jwtTokenProvider, refreshTokenStore, userDetailsService, I18nUtils.resolve(i18nProvider));
    }
    ...
    @EnableJpaRepositories(basePackages = "studio.echo.base.security.jwt.refresh")
