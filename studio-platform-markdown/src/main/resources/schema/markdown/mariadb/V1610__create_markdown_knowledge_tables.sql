...
CREATE TABLE IF NOT EXISTS tb_ai_markdown_revision (
    ...
    updated_at TIMESTAMP(6) NOT NULL,
    INDEX idx_ai_markdown_revision_document (document_id, created_at),
    INDEX idx_ai_markdown_revision_convert_job (document_convert_job_id),
    INDEX idx_ai_markdown_revision_dedupe
        (source_attachment_id, source_content_hash, extractor_type, extractor_version, options_hash, status)
);
...
CREATE TABLE IF NOT EXISTS tb_ai_markdown_locator (
    ...
    INDEX idx_ai_markdown_locator_revision (revision_id, start_offset)
);
...
CREATE TABLE IF NOT EXISTS tb_ai_markdown_resource (
    ...
    metadata_json LONGTEXT,
    INDEX idx_ai_markdown_resource_revision (revision_id)
);
