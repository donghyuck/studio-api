...
기능 활성화는 `studio.features.textract.enabled`가 담당하고, 텍스트 추출 런타임 정책은 `studio.textract.*`를 사용한다.
기존 `studio.features.text.*`와 `studio.text.*`는 1-2 릴리스 동안 fallback으로 유지되며, 사용 시 startup WARN 로그가 출력된다.
...
- `Tesseract` 엔진 설치
- `tessdata` 언어 데이터 설치
- `studio.textract.tesseract.datapath`가 `tessdata` 디렉터리를 가리키도록 설정
...
| Legacy | Target |
| --- | --- |
| `studio.features.text.enabled` | `studio.features.textract.enabled` |
| `studio.text.max-extract-size` | `studio.textract.max-extract-size` |
| `studio.text.tesseract.datapath` | `studio.textract.tesseract.datapath` |
| `studio.text.tesseract.language` | `studio.textract.tesseract.language` |
| `studio.features.text.max-extract-bytes` | `studio.textract.max-extract-size` |
| `studio.features.text.tesseract.datapath` | `studio.textract.tesseract.datapath` |
| `studio.features.text.tesseract.language` | `studio.textract.tesseract.language` |

예전 byte 기반 설정은 다음처럼 읽히지만, 새 설정이 있으면 새 설정이 우선한다.
...
studio:
  features:
    text:
      enabled: true
      # deprecated: studio.textract.max-extract-size 사용
      max-extract-bytes: 52428800
      tesseract:
        # deprecated: studio.textract.tesseract.datapath 사용
        datapath: /opt/homebrew/share/tessdata
        # deprecated: studio.textract.tesseract.language 사용
        language: kor+eng
```
