...
## Package structure
Wiki 모듈은 `domain / application / infrastructure / web` 기준으로 재배치되었다. 이 변경은 breaking rename이며 기존
`studio.one.application.wiki.model`, `permission`, `service`, `exception`, `persistence.jpa`, `web.dto` wrapper는 제공하지 않는다.

- `domain.model`: page/revision 모델과 Wiki permission action
- `application.command`: page write/archive/revert command
- `application.usecase`: `WikiPageService`, `WikiRenderService`
- `application.service`: JPA 기반 service 구현과 `WikiWorkspacePermissionContributor`
- `application.error`: Wiki error code와 exception
