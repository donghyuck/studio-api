package studio.one.platform.objecttype.application.service;
...
import studio.one.platform.objecttype.domain.model.ObjectTypeStatus;
    ...
    static void validateStatus(String status) {
        if (ObjectTypeStatus.from(status) == null) {
            throw studio.one.platform.exception.PlatformRuntimeException.of(
                    studio.one.platform.objecttype.domain.error.ObjectTypeErrorCodes.VALIDATION_ERROR,
                    "status");
        }
    }
