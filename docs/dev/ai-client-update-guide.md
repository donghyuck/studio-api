...
| RAG 채팅 | `objectType`, `objectId`, `ragQuery`, `topK`, `minScore`, `debug` 흐름 확인 |
...
| RAG object 색인 삭제 | `DELETE` | `/api/mgmt/ai/rag/objects/{objectType}/{objectId}` |
...
9. 객체 색인 삭제 버튼은 `services:ai_rag write` 권한이 있을 때만 활성화한다. `objectType=attachment`이면 `features:attachment write`도 필요하다.

`DELETE /api/mgmt/ai/rag/objects/{objectType}/{objectId}`는 해당 object scope의 chunk/vector/metadata를 삭제한다.
같은 object scope에 `PENDING` 또는 `RUNNING` job이 있으면 서버는 `409 Conflict`를 반환하므로 클라이언트는
먼저 취소하거나 완료될 때까지 대기한다. 삭제 후 metadata API는 `data.indexed=false`를 반환하고,
`/chunks/page`는 빈 `items`를 반환해야 한다.
...
- `topK`는 화면의 최대 참고 문서 수와 맞춘다. 기존 `ragTopK`도 호환용으로 동작하지만 새 클라이언트는 `topK`를 사용한다.
- `minScore`는 최종 결과 cutoff다. 요청하지 않으면 서버 설정 `studio.ai.rag.retrieval.min-score`가 적용되고,
  필터 후 결과 수는 `topK`보다 적을 수 있다.
- fallback 전략 선택은 서버 설정 `min-relevance-score` 기준으로 결정되며, client `minScore`는 최종 반환/context 후보 cutoff로만 적용된다.
...
- `topK`/`minScore`를 생략하면 기존 `topK=5`, `minScore` 미적용 대신
  `studio.ai.rag.retrieval.top-k`, `studio.ai.rag.retrieval.min-score` 설정값이 적용된다.
...
화면에는 strategy, result count, requested/effective threshold, weight, object scope, topK, minScore 적용 전후 count,
context expansion 적용 여부 같은 운영 정보만 표시한다.
