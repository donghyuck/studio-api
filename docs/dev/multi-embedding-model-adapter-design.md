...
`defaultChatPort`는 `AiProviderRegistry.chatPort(null)`로 만든다. `studio.ai.default-provider`만 설정하면
두 `null` lookup 모두 legacy fallback provider를 사용한다. chat과 embedding을 분리해야 하면
`studio.ai.default-chat-provider`와 `studio.ai.default-embedding-provider`를 지정한다. 기존
...
재사용하므로 provider id를 나눠도 실제 model이 나뉘지 않는다. 위 예시는 auto-config 부팅 전제를 설명하기 위한
목표 형태이며, provider별 runtime model/options 주입은 아래 첫 구현 PR에서 먼저 보강해야 한다.
