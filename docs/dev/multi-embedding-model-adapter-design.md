...
- `starter/studio-platform-starter-ai`
  ...
  - `LangChainEmbeddingAdapter`: 생성 시점에 구성된 LangChain4j `EmbeddingModel` 하나를 호출
...
LangChain4j `EmbeddingModel` 자체가 단일 model로 생성되어 있으므로 같은 provider 안에서 request마다 다른 model을
...
auto-config는 `defaultEmbeddingPort`를 `AiProviderRegistry.embeddingPort(null)`로 만들고,
`defaultChatPort`는 `AiProviderRegistry.chatPort(null)`로 만든다. `studio.ai.routing.default-chat-provider`와 `studio.ai.routing.default-embedding-provider`가 기본 lookup에 사용된다. legacy `studio.ai.default-provider`만 설정하면
두 `null` lookup 모두 fallback provider를 사용한다. 기존
`default-provider`를 함께 설정하는 경우 해당 provider는 하위 호환을 위해 chat/embedding port를 모두 제공해야 한다.
...
studio:
  ai:
    routing:
      ...
      default-embedding-provider: gemini-embed-768
    providers:
      gemini-chat:
...
현재 1.x LangChain4j adapter는 provider id별 runtime model/options를 직접 구성한다. 따라서 provider id를 model별로 분리하면
실제 `EmbeddingModel`도 분리된다. provider별 dimension은 `studio.ai.providers.<id>.embedding.dimension`과
`studio.ai.rag.embedding-profiles.*.dimension`을 일치시킨다.
...
`providerId:modelId`는 registry 내부 alias로만 사용한다. RAG metadata에 저장되는 `embeddingProvider`와
`embeddingModel`은 사용자가 설정한 logical provider id와 실제 model name 의미를 유지해야 하며, resolver가
lookup 편의를 위해 이 값을 synthetic key로 바꾸면 안 된다.
...
1. `AiAdapterProperties.Provider.Channel`에 optional `models` map과 model config object를 추가한다.
   model config는 최소 `model`, `dimension`을 담고, provider-specific options는 provider adapter가 해석하는
   별도 map 또는 typed option object로 분리한다.
2. `ProviderEmbeddingPortFactory`에 model-specific port 생성에 필요한 factory contract를 추가한다.
   예를 들어 `create(provider, modelId, modelConfig, env, embeddingModelProvider)`처럼 실제 model name,
   dimension, provider-specific option을 factory 입력으로 전달한다.
3. 한 provider type만 먼저 지원한다. Google GenAI 또는 Ollama처럼 starter가 직접 `EmbeddingModel`을 build하는
...
5. `RagEmbeddingProfileResolver`가 profile의 provider/model을 registry 내부 alias로 조회하되, 저장 metadata의
   `embeddingProvider` / `embeddingModel` 값은 request/profile의 logical 값을 유지한다.
