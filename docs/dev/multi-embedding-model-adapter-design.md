...
`defaultChatPort`도 `AiProviderRegistry.chatPort(null)`로 만든다. 두 `null` lookup은 모두
`studio.ai.default-provider`를 사용하므로, 현재 부팅 가능한 설정에서는 `default-provider`가
chat과 embedding port를 모두 제공해야 한다. chat default와 embedding default를 분리하려면
`studio.ai.default-chat-provider` / `studio.ai.default-embedding-provider` 같은 선행 계약 변경이 필요하다.

```yaml
studio:
  ai:
    default-provider: gemini-default
    providers:
      gemini-default:
