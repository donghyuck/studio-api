...
public class DefaultChunkingOrchestrator implements ChunkingOrchestrator {
    ...
    public List<Chunk> chunk(ChunkingContext context) {
        ...
        if (!usesDocumentAwareChunker(effectiveContext.strategy())
    ...
    private Chunker selectChunker(ChunkingContext context) {
        ...
        if (strategy == ChunkingStrategyType.KNOWLEDGE_BLOCK && !properties.getKnowledgeBlock().isEnabled()) {
            throw new IllegalStateException(
                    "Knowledge block chunking is disabled. Set studio.chunking.knowledge-block.enabled=true to use knowledge-block.");
        }
        if (strategy != ChunkingStrategyType.FIXED_SIZE
            ...
            throw new IllegalArgumentException(
                    "Unsupported pure chunking strategy: " + strategy
                            + ". Supported values are FIXED_SIZE, RECURSIVE, STRUCTURE_BASED, BLOCKIFY, and KNOWLEDGE_BLOCK.");
        }
    ...
    private boolean usesDocumentAwareChunker(ChunkingStrategyType strategy) {
        return strategy == ChunkingStrategyType.BLOCKIFY
                || strategy == ChunkingStrategyType.KNOWLEDGE_BLOCK;
    }
}
